...
export default function SearchContainer({ searchContainerRef }: Props) {
  ...
  const isFarmRoute = shopRoutes.some((item) => pathname.includes(item));
  ...
  const {
      ...
      }),
    {
      ...
      retry: false,
      enabled: !!debouncedSearchTerm && !isFarmRoute,
    }
  );
  ...
  const {
    ...
    ({ pageParam = 1 }) =>
      productService.search({
        search: debouncedSearchTerm,
        page: pageParam,
        shop_id: isFarmRoute ? query.id : undefined,
        address: location,
      }),
    {
      getNextPageParam: (lastPage: any) => {
