...
import courierOrderService from '../../services/courier/order';
...
export const fetchPackeringOrders = createAsyncThunk(
  'order/fetchPackeringOrders',
  (params = {}) => {
    return orderService
      .getAll({ ...initialState.packering.params, ...params })
      .then((res) => res.data);
  },
);
...
const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //fetch packering orders
    builder.addCase(fetchPackeringOrders.pending, (state) => {
      state.packering.loading = true;
    });
    builder.addCase(fetchPackeringOrders.fulfilled, (state, action) => {
      const { payload } = action;
      state.packering.loading = false;
      state.items = {
        ...state.items,
        packering: [...state.items.packering, ...payload.orders],
      };
      state.packering.meta = payload.meta;
      state.packering.params.page = payload.meta.current_page;
      state.packering.params.perPage = payload.meta.per_page;
      state.packering.error = '';
    });
    builder.addCase(fetchPackeringOrders.rejected, (state, action) => {
      state.packering.loading = false;
      state.items.packering = [];
      state.packering.error = action.error.message;
    });
    ...
    builder.addCase(fetchCourierOrders.pending, (state) => {
      state.loading = true;
    });
    builder.addCase(fetchCourierOrders.fulfilled, (state, action) => {
      const { payload } = action;
      state.loading = false;
      state.orders = payload.data;
      state.meta = payload.meta;
