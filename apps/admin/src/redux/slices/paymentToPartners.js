...
import paymentFromPartnerDeliveryService from 'services/courier/payment-from-partner'
...
export const fetchPaymentToPartnersList = createAsyncThunk(
  'paymentToPartners',
  (params = {}) => paymentToPartnerService.getAllTransactions(params).then((res) => res)
);
...
export const fetchCourierPaymentFromPartners = createAsyncThunk(
  'courierPaymentFromPartners',
  (params = {}) => paymentFromPartnerDeliveryService.getAll(params).then((res) => res)
);
...
const paymentToPartnersSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchPaymentToPartnersCompletedList.fulfilled, (state, action) => {
      const { payload } = action;
      state.loading = false;
    ...
    builder.addCase(fetchPaymentToPartnersCompletedList.rejected, (state, action) => {
      state.loading = false;
      state.list = [];
      state.error = action.error.message;
    });
    ...
    builder.addCase(fetchCourierPaymentFromPartners.pending, (state) => {
      state.loading = true;
    });
    builder.addCase(fetchCourierPaymentFromPartners.fulfilled, (state, action) => {
      const { payload } = action;
      state.loading = false;
      state.list = payload.data;
      state.meta = payload.meta;
    ...
    builder.addCase(fetchCourierPaymentFromPartners.rejected, (state, action) => {
      state.loading = false;
      state.list = [];
      state.error = action.error.message;
    });
