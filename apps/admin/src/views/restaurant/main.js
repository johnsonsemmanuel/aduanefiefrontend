...
import FarmAddData from './farm-add-data';
...
const FarmMain = ({ next, action_type = '', user }) => {
  ...
  const onFinish = (values) => {
    ...
    if (action_type === 'edit') {
      farmUpdate(values, body);
    } else {
      farmCreate(values, body);
    }
  };

  function farmCreate(values, params) {
    farmService
      .create(params)
      .then(({ data }) => {
        dispatch(
        ...
        navigate(`/farm/${data.uuid}?step=1`);
      })
      .catch((err) => console.error(err))
      .finally(() => setLoadingBtn(false));
  }

  function farmUpdate(values, params) {
    farmService
      .update(uuid, params)
      .then(() => {
        dispatch(
