...
import FarmStatusModal from './farm-status-modal';
...
const Farms = () => {
  ...
  const goToClone = (row) => {
    dispatch(
      addMenu({
        id: 'farm-clone',
        url: `farm-clone/${row.uuid}`,
        name: t('farm.clone'),
      })
    );
    navigate(`/farm-clone/${row.uuid}`);
  };
    ...
    >
      ...
      {restore && (
        <ResultModal
          ...
          click={restore.restore ? farmRestoreAll : farmDropAll}
