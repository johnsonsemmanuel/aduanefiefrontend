...
export default function DeliveryMapOrders() {
  ...
  const courier = activeMenu?.data?.courier;
  ...
  const handleLoadMap = (map, maps) => {
    const markers = [
      {
        lat: Number(
          courier?.delivery_man_setting?.location?.latitude || '0',
        ),
        lng: Number(
          courier?.delivery_man_setting?.location?.longitude || '0',
        ),
      },
      {
      ...
      <Row gutter={8}>
        ...
        <Col span={18}>
          <Card className='map-user-card with-order'>
            <OrderData data={courier} order={data} />
          </Card>
          <div
