...
import { fetchCourierRequest } from 'redux/slices/courier-request';
import useDidUpdate from 'helpers/useDidUpdate';
import formatSortType from 'helpers/formatSortType';
import { EditOutlined, EyeOutlined } from '@ant-design/icons';
import CourierRequestModal from './details-modal';
...
export default function CourierRequest() {
  ...
  const { courierRequestData, meta, loading } = useSelector(
    (state) => state.courierRequest,
    shallowEqual,
  );
  ...
  useDidUpdate(() => {
    dispatch(fetchCourierRequest(paramsData));
  }, [activeMenu?.data]);
  ...
  return (
    <Card title={t('courier.request')}>
      <Table
        scroll={{ x: true }}
        rowSelection={rowSelection}
        columns={columns?.filter((item) => item?.is_show)}
        dataSource={courierRequestData}
        pagination={{
