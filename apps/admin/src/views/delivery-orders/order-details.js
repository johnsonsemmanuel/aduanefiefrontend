...
import orderService from '../../services/courier/order';
  ...
  const expandedRowRender = (row) => {
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'quantity',
        key: 'quantity',
        render: (_, row) => <span>{row.quantity * (row?.stock?.product?.interval || 1)}{row?.stock?.product?.unit?.translation?.title}</span>
      },
      {
        title: t('discount'),
        dataIndex: 'discount',
