...
import OrderCourier from '../order/orderCourier';
...
export default function DeliveryOrder() {
  ...
  useDidUpdate(() => {
    const paramsData = {
      ...
      page: data?.page,
      courier: data?.courier?.value,
      date_from: data?.date_from,
      date_to: data?.date_to,
      status: data?.status === 'un_assigned_order' ? undefined : data?.status,
      'isset-courier': data?.status === 'un_assigned_order' ? 1 : 0,
    };
      ...
      <Card className='p-0'>
        <Space wrap size={[12, 20]}>
          ...
          <DebounceSelect
            ...
            allowClear={true}
            onChange={(value) => handleFilter({ courier: value })}
            value={activeMenu.data?.courier}
          />
