...
import { fetchCourierOrders } from 'redux/slices/orders';
...
export default function CourierOrders() {
  ...
  const goToShow = (row) => {
    dispatch(
      addMenu({
        url: `courier/order/details/${row.id}`,
        id: 'order_details',
        name: t('order.details'),
      }),
    );
    navigate(`/courier/order/details/${row.id}`);
  };
  ...
  useDidUpdate(() => {
    dispatch(fetchCourierOrders(paramsData));
  }, [activeMenu?.data]);
