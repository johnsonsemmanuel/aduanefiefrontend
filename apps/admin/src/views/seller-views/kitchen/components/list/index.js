...
import { fetchSellerPackhouses } from 'redux/slices/packhouse';
...
const PackhouseList = ({ id = null, setId = (a) => {} }) => {
  ...
  const { packhouses, loading, meta, initialParams } = useSelector(
    (state) => state.packhouse.seller,
    shallowEqual,
  );
  ...
  const fetch = (params = {}) => {
    batch(() => {
      dispatch(fetchSellerPackhouses(params));
  ...
  const goToEdit = (id) => {
    const url = `seller/packhouse/edit/${id}`;
    dispatch(
      addMenu({
        id: 'edit-packhouse',
        url,
