...
const ProductFinish = ({ data, mainInfoChangedData, isRequest, prev }) => {
  ...
  const finish = () => {
    ...
    if (mainInfoChangedData) {
      requestModelsService
        ...
        .then(() => {
          toast.success(t('request.successfully.created'));
          dispatch(removeFromMenu({ ...activeMenu, nextUrl }));
          navigate(`/${nextUrl}`);
        });
    }
  ...
  const productInfo = mainInfoChangedData;
  ...
  return (
    <>
      {Boolean(productInfo) && (
        <>
          <Descriptions bordered title={t('changed.info')}>
            ...
            <Descriptions.Item label={t('packhouse')} span={1} column={1}>
              {productInfo?.packhouse?.label || t('N/A')}
            </Descriptions.Item>
      ...
      <Descriptions bordered title={t('original.info')}>
        ...
        <Descriptions.Item label={t('packhouse')} span={1} column={1}>
          {data?.packhouse?.translation?.title || t('N/A')}
        </Descriptions.Item>
        <Descriptions.Item label={t('brand')} span={1} column={1}>
      ...
      <Space className='w-100 justify-content-end'>
        ...
        <Button loading={buttonLoading} type='primary' onClick={finish}>
          {t('finish')}
        </Button>
      </Space>
    </>
