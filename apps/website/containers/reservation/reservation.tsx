...
export default function Reservation({ data }: Props) {
  ...
  const {
    ...
    () => bookingService.getZones({ page: 1, perPage: 100, shop_id }),
    {
      select: (data) =>
        data.data.map((item) => ({
  ...
  const formik = useFormik({
    ...
    validate: (values: formValues) => {
      ...
      if (
        ...
        (values.number_of_people < 1 ||
          values.number_of_people >
            (tables?.find((item) => item?.value === values?.table_id)
              ?.chairCount ?? 0))
      ) {
  ...
  const handleSubmit = (selectedValue: string) => {
    ...
    formik.setFieldValue("zone_id", undefined);
