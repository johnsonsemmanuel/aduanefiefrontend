...
import 'package:aduanefie/domain/handlers/api_result.dart';
...
class RegisterConfirmationNotifier extends Notifier<RegisterConfirmationState> {
  ...
  Future<void> confirmCodeResetPassword(
    ...
    if (connected) {
      ...
      final response = await authRepository.forgotPasswordConfirm(
        verifyCode: state.confirmCode.trim(),
        email: email,
      );
      response.when(
    ...
    if (connected) {
      ...
      if (AppConstants.isPhoneFirebase) {
        try {
          ...
          final response = await authRepository.forgotPasswordConfirmWithPhone(
            phone: phone,
          );
          response.when(
            success: (data) async {
  ...
  Future<void> resendConfirmation(
    ...
    if (connected) {
      ...
      if (isResetPassword) {
        ...
        response = await authRepository.sigUp(email: email.trim());
      }
  ...
  Future<void> sendCodeToNumber(
    ...
    if (connected) {
      ...
      if (AppConstants.isPhoneFirebase) {
        ...
        final response = await authRepository.sendOtp(phone: phoneNumber);
        response.when(
          success: (success) {
            state = state.copyWith(
              isResending: false,
