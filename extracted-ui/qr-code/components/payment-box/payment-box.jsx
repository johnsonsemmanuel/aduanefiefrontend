...
export function PaymentBox({
  ...
  const [isOrderRefetching, setIsOrderRefetching] = useState(false);
  ...
  const { isLoading: externalPayLoading, mutate: externalPay } =
    useExternalPayment({
      onSuccess: (res, body) => {
        window.sessionStorage.setItem("selected-method", body.paymentType);
        onExternalPaymentSuccess(res?.data);

        if (Array.isArray(res?.data) && res?.data?.length > 1) {
          setIsOrderRefetching(true);
          refetchOrder().finally(() => setIsOrderRefetching(false));
          setSplitPaymentLinks(res.data);
        }
      },
    });
  ...
  const isLoading =
    externalPayLoading || isTransactionLoading || isOrderRefetching;
  const handleCreatePayment = (method, splitCount = minSplit) => {
    ...
    if (isExternal) {
      externalPay({
        processType: data?.split ? "split" : "order",
        paymentType: method,
        body: data,
      });
    } else {
      ...
      <Container maxWidth="sm" className={classes.container}>
        ...
        <Stack spacing={1} px={1}>
          ...
          <div className={classes.btnWrapper}>
            <Button
              disabled={!selectedPayment || selectedPayment === "cash"}
              fullWidth
              variant="outlined"
              size="large"
