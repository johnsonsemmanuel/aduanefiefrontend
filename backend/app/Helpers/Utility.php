...
class Utility
{
    ...
    public function calculateMinuteByKm(float $distanceKm): float
    {
        // время движения без остановок (в минутах)
        $driveMinutes = ($distanceKm / 55) * 60;

        // количество светофоров
        $lights = $distanceKm / 2;

        // ожидаемое число красных
        $redLights = $lights * 0.5;

        // задержка на светофорах (в минутах)
        $delayMinutes = ($redLights * 50) / 60;

        return ceil($driveMinutes + $delayMinutes);
    }
