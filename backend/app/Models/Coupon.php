 ...
 * @method static Builder|self checkCoupon(string $coupon, string|int|null $shopId, bool $forParcel = false)
...
class Coupon extends Model
{
    ...
    public static function scopeCheckCoupon($query, $coupon, $shopId = null, bool $forParcel = false) {
        return $query
            ->where('name', '=', $coupon)
            ->when($shopId, fn($q) => $q->where('shop_id', $shopId))
            ->where('qty', '>', 0)
            ->where('for_parcel', $forParcel)
            ->whereDate('expired_at', '>', now());
    }

    public function scopeFilter($query, $array): void
    {
        $query
            ->when(data_get($array, 'shop_id'), function ($q, $shopId) {
                $q->where('shop_id', $shopId);
            })
            ->when(data_get($array, 'active'), function ($q) {
                $q->where('qty', '>', 0)->whereDate('expired_at', '>', now());
            })
            ...
            ->when(isset($array['for_parcel']), fn($q) => $q->where('for_parcel', $array['for_parcel']))
