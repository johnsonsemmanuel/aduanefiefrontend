...
class Product extends Model
{
    ...
    public function scopeFilter($query, $array)
    {
        ...
        $query
			...
			->when(data_get($array, 'type'), fn($q, $value) => $q->where('type', $value))
			->when(data_get($array, 'expired_at_from'), fn($q, $value) => $q->where('expired_at', '>=', $value))
			->when(data_get($array, 'expired_at_to'),   fn($q, $value) => $q->where('expired_at', '<=', $value))
			->when(isset($array['shop_id']), function ($q) use ($array) {
                $q->where('shop_id', $array['shop_id']);
            })
			->when(data_get($array, 'latitude') && data_get($array, 'longitude'), function ($q) use ($array) {
                $q->whereHas('shop', fn($q) => $q->filter(['address' => ['latitude' => $array['latitude'], 'longitude' => $array['longitude']]]));
            })
