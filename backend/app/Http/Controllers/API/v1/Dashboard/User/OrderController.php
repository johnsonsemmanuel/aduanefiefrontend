...
class OrderController extends UserBaseController
{
     ...
     * @return AnonymousResourceCollection
     */
	public function customOrderDetailsByShop(int $id, FilterParamsRequest $request): AnonymousResourceCollection
    {
        $latestIds = OrderDetail::whereHas('order', fn($q) => $q
            ->where('user_id', auth('sanctum')->id())
            ->where('shop_id', $id)
        )
        ...
        $models = OrderDetail::whereIn('id', $latestIds)
            ->orderBy('id', 'desc')
            ->paginate($request->input('perPage', 10));

        return OrderDetailResource::collection($models);
	}
}
