...
class BackupController extends AdminBaseController
{
    ...
    {
        if (!Storage::exists('aduanefie')) {
            Storage::makeDirectory('aduanefie');
        }

//        Artisan::call('backup:run');

        $path = Storage::disk('local')->path('aduanefie');
        ...
        foreach ($files as $item) {
            ...
            $title = Str::of($item->getPathname())->afterLast('/')->before('.');

            $result = BackupHistory::updateOrCreate([
                'title' => $title
            ], [
                'status' => true,
                'path' => "/backup/download/$title",
        ...
        return $this->successResponse('Backup was successfully', [
            'title' => $result->title,
            'path' => '/backup/download/' . $result->title,
        ]);
    }
