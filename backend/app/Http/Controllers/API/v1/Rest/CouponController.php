...
class CouponController extends RestBaseController
{
    ...
    {
        $coupons = $this->repository->couponsList($request->merge(['active' => true])->all());
    ...
    {
        $coupon = Coupon::checkCoupon(
            $request->input('coupon'),
            $request->input('shop_id'),
            $request->input('for_parcel', false)
        )->first();
        ...
        $result = OrderCoupon::where(function ($q) use ($request) {
            $q->where('user_id', $request->input('user_id'))->orWhere('user_id', auth('sanctum')->id());
        })
