...
class OrderRepository extends CoreRepository implements OrderRepoInterface
{
	...
	public function orderStocksCalculate(array $filter): array
	{
        ...
        $customKeyCount = $products->count();

        foreach ($products->whereNull('stock_id') as $product) {

//            $customKeyCount++;

            $result[$customKeyCount++] = [
                'id'                 => null,
                'price'              => $product['custom_price'] ?? 0,
                'quantity'           => $product['quantity'] ?? 1,
                'countable_quantity' => $product['quantity'] ?? 1,
                'tax'                => 0,
                'discount'           => 0,
                'total_price'        => $product['custom_price'] ?? 1,
                'stock'              => null,
                'addons'             => [],
                'custom_name'        => $product['custom_name'] ?? null,
                'custom_unit_id'     => $product['custom_unit_id'] ?? null,
                'custom_photo'       => $product['custom_photo'] ?? null,
                'custom_by'          => isset($product['custom_unit_id']) ? 'amount' : 'quantity',
...
//			$this->addBonusCalculate($quantity, $result, $bonusShop, Bonus::BONUS_TYPE_SHOP);
        ...
        $totalPrice = collect(array_values($result))->sum('total_price');

        $shop = Shop::with([
            'translation' => fn($q) => $q->where('locale', $this->language),
            'bonus'       => fn($q) => $q->where('type',Bonus::TYPE_SUM)->where('expired_at','>=', now())
	...
	public function orderById(int $id, ?int $shopId = null, ?int $userId = null, ?string $phone = null, ?string $email = null): ?Order
	{
		return $this->model()
			->with([
				'user' => fn($u) => $u
                    ->withCount(['orders' => fn($u) => $u->where('status', Order::STATUS_DELIVERED)])
					->withSum(['orders' => fn($u) => $u->where('status', Order::STATUS_DELIVERED)], 'total_price'),
				'review' => fn($q) => $q->when($userId, fn($q) => $q->whereHas('user', fn($q) => $q->where('id', $userId))),
				'pointHistories',
				'currency',
				'deliveryMan' => fn($d) => $d->withAvg('assignReviews', 'rating')->withCount('deliveryManOrders'),
