...
class TableRepository extends CoreRepository
{
    ...
    public function paginate(array $filter = []): LengthAwarePaginator
    {
        ...
        return $models
            ...
            ->with([
                ...
                'shopSection.translation' => fn($q) => $q->where('locale', $this->language),
            ])
    ...
    public function show(Table $model): Table|null
    {
        return $model->loadMissing([
            'shopSection.translation'       => fn($q) => $q->where('locale', $this->language),
            'shopSection.shop.translation'  => fn($q) => $q->where('locale', $this->language)
        ]);
    }
    ...
    {
        $minTime  = Settings::where('key', 'min_reservation_time')->first()?->value;
