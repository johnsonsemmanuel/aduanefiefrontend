...
//        }
    ...
    public function branchProducts(array $filter): AnonymousResourceCollection
    {
        ...
        $categories = $this->firstRememberProduct($filter, $shopId, $productPerPage);
     ...
     */
    private function firstRememberProduct(array $filter, ?int $shopId, int $productPerPage): array|Collection
    {
    ...
    public function products(array $filter): array
    {
        ...
        $categories = Category::where([
            ...
            ->when($categoryIds,
                ...
                fn($q, $id) => $q->whereIn('type', [Category::MAIN, Category::COMBO]),
            )
            ->with([
                'translation' => fn($q) => $q->where('locale', $this->language)
                    ->select('id', 'locale', 'title', 'category_id'),
    ...
    public function categories(array $filter): LengthAwarePaginator
    {
        ...
        return Category::filter($filter)
            ...
            ->whereHas('translation', fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)))
            ->where(function ($q) use ($filter) {
                $q
                    ->whereHas('products', fn($q) => $q->where('shop_id', data_get($filter, 'shop_id')))
                    ->orWhereHas('children.products', fn($q) => $q->where('shop_id', data_get($filter, 'shop_id')));
            })
            ->orderBy('id')
            ->paginate(data_get($filter, 'perPage', 10));
    }
