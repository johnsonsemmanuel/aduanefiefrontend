...
class CartService extends CoreService
{
    ...
    public function insertProducts(array $data): array
    {
        ...
        $cart = $this->model()->with([
            ...
            'userCarts.cartDetails.customUnit',
        ])
            ->firstOrCreate([
                'owner_id' => $userId,
    ...
    private function addCustomProducts(array $data, UserCart $userCart): void
    {
        if (isset($data['custom_products_delete_ids'])) {
            DB::table('cart_details')
                ->where('user_cart_id', $userCart->id)
                ->whereIn('id', $data['custom_products_delete_ids'])
                ->delete();
        }

        $products = collect(data_get($data, 'products', []))->whereNull('stock_id');

        foreach ($products as $product) {

            CartDetail::updateOrCreate([
                'id' => $product['custom_id'] ?? null
            ], [
                'user_cart_id'   => $userCart->id,
                'stock_id'	     => null,
                'parent_id'	     => null,
    ...
    private function collectProducts(array $data, Cart $cart, UserCart $userCart): int
    {
        try {
            return DB::transaction(function () use ($data, $cart, $userCart) {
                ...
                $products = collect(data_get($data, 'products', []))->whereNotNull('stock_id');

                $this->addCustomProducts($data, $userCart);
    ...
    private function successReturn(int $cartId): array
    {
        ...
        $cart = $this->model()->with([
            ...
            'userCarts.cartDetails' => fn($q) => $q->whereNull('parent_id'),
            'userCarts.cartDetails.customUnit.translation' => fn($q) => $q->where('locale', $this->language),
