...
class SmsPayloadService extends CoreService
{
    ...
    public function prepareValidate($data): array
    {
        if (data_get($data, 'type') === SmsPayload::FIREBASE) {
            ...
            return ['status' => true];
        } else if (data_get($data, 'type') === SmsPayload::MTC) {

            $validator = $this->mtc($data);
     ...
     * @return \Illuminate\Contracts\Validation\Validator|\Illuminate\Validation\Validator
     */
    public function mtc(array $data): \Illuminate\Contracts\Validation\Validator|\Illuminate\Validation\Validator
    {
        return Validator::make($data, [
            'payload.api_key'      => ['required', 'string'],
            'payload.phone_number' => ['required', 'string'],
