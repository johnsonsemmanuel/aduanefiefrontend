...
class OrderDetailService extends CoreService
{
	...
	{
        foreach (collect($collection)->whereNull('stock_id') as $item) {
            /** @var OrderDetail $orderDetail */
            $order->orderDetails()->create([
                'custom_status_note' => null,
                'custom_name'        => $item['custom_name'] ?? null,
                'custom_unit_id'     => $item['custom_unit_id'] ?? null,
                'custom_photo'       => $item['custom_photo'] ?? null,
                'note'               => $item['note'] ?? '',
                'origin_price'       => $item['custom_price'] ?? 0,
                'tax'                => 0,
                'discount'           => 0,
                'total_price'        => $item['custom_price'] ?? 0,
                'stock_id'           => null,
                'parent_id'          => null,
        ...
        foreach (collect($collection)->whereNotNull('stock_id') as $item) {
    ...
    public function createOrderUser(Order $order, int $cartId, ?array $notes = []): Order
    {
        ...
        foreach ($cart->userCarts as $userCart) {
			...
			foreach ($cartDetails as $cartDetail) {
                ...
                if ($cartDetail->custom_name) {
                    $order->orderDetails()->create([
                        'custom_status_note' => null,
                        'custom_name'        => $cartDetail->custom_name,
                        'custom_unit_id'     => $cartDetail->custom_unit_id,
                        'custom_photo'       => $cartDetail->custom_photo,
                        'note'               => $notes[$cartDetail->id] ?? '',
                        'origin_price'       => $cartDetail->price,
                        'tax'                => 0,
                        'discount'           => 0,
                ...
                $cartDetail->setAttribute('note', $notes[$cartDetail->id] ?? '');
				...
				foreach ($cartDetail->children as $addon) {
					...
					$addon->setAttribute('note', $notes[$addon->id] ?? '');
...
//            }
    ...
    public function statusUpdate(OrderDetail $orderDetail, array $data): array
    {
        $status = $data['status'];
        ...
        $orderDetail->update([
            'status' => $status,
            'custom_status_note' => $data['custom_status_note'] ?? $orderDetail->custom_status_note
        ]);
