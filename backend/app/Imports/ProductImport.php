...
class ProductImport extends BaseImport implements ToCollection, WithHeadingRow, WithBatchInserts, WithChunkReading
{
    ...
    public function collection(Collection $collection): void
    {
        foreach ($collection as $row) {
            ...
            DB::transaction(function () use ($row) {
                ...
                $status = $row['status'] ?? Product::PENDING;
                ...
                $data = [
                    ...
                    'status'      => in_array($status, Product::STATUSES) ? $status : Product::PENDING,
	...
	private function createAddons(Stock $stock, array $stocks, bool $isAddon = false, ?string $groupKey = ''): void
	{
		...
		foreach ($addonGroups as $addonKey => $addonGroup) {
            ...
            $status = $addonGroup["{$addonKey}_status"] ?? Product::PENDING;

			$addon = Product::updateOrCreate([
				'id' 		=> data_get($addonGroup, "{$addonKey}_id")
			], [
				'interval'	=> $addonGroup["{$addonKey}_interval"] ?? 1,
				'tax'		=> data_get($addonGroup, "{$addonKey}_tax"),
				'active'	=> data_get($addonGroup, "{$addonKey}_active") === 'active' ? 1 : 0,
				'status'	=> in_array($status, Product::STATUSES) ? $status : Product::PENDING,
			...
			$addon->stock()->updateOrCreate([
				...
				'price'		=> $addonGroup["{$addonKey}_price"] ?? 0,
				'quantity'	=> $addonGroup["{$addonKey}_quantity"] ?? 0,
				'sku'		=> $addonGroup["{$addonKey}_sku"] ?? 0,
				'addon'		=> true,
                'img'	    => $addonGroup["{$addonKey}_img"] ?? '',
			]);
